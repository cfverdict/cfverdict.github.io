...
async function getUserInfo(handle) {
    const response = await __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$axios$2f$lib$2f$axios$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["default"].get(`${CF_API_BASE}/user.info`, {
        params: {
            handles: handle
        }
    });
    if (response.data.status !== 'OK') {
        throw new Error(`Failed to fetch user info: ${response.data.comment}`);
    }
    return response.data.result[0];
}
async function getUserSubmissions(handle) {
    // Fetching all submissions. For a real app, you might want to paginate or limit.
    const response = await __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$axios$2f$lib$2f$axios$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["default"].get(`${CF_API_BASE}/user.status`, {
        params: {
            handle
        }
    });
    if (response.data.status !== 'OK') {
        throw new Error(`Failed to fetch submissions: ${response.data.comment}`);
    }
    return response.data.result;
}
async function getUserRatingHistory(handle) {
    const response = await __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$axios$2f$lib$2f$axios$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["default"].get(`${CF_API_BASE}/user.rating`, {
        params: {
            handle
        }
    });
    if (response.data.status !== 'OK') {
        throw new Error(`Failed to fetch rating history: ${response.data.comment}`);
    }
    return response.data.result;
}
}),
...
//# sourceMappingURL=%5Broot-of-the-server%5D__b9205d3e._.js.map